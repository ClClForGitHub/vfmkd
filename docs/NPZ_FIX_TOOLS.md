...
### 1. `tools/bulk_fix_npz_features.py`
...
**使用示例**:
```bash
python tools/bulk_fix_npz_features.py \
...
### 2. `tools/fix_npz_features_inplace.py`
...
**使用示例**:
```bash
python tools/fix_npz_features_inplace.py \
...
**使用示例**:
```bash
python tools/fix_train_test_npz.py \
...
### 4. `tools/core/update_edge_maps_from_npz.py`
...
**使用示例**:
```bash
python tools/core/update_edge_maps_from_npz.py \
...
|------|------|----------|------|----------|------|
| `bulk_fix_npz_features.py` | 特征修复 | 内部实现（SAM2底层API） | `sam2.build_sam`, `SAM2Transforms` | `feature_flag=1` | 递归扫描，自动去重 |
| `fix_npz_features_inplace.py` | 特征修复 | 内部实现（SAM2底层API） | `sam2.build_sam`, `SAM2Transforms` | 无 | 基于样本清单，支持分散存储 |
| `fix_train_test_npz.py` | 特征修复 | 内部实现（SAM2底层API） | `sam2.build_sam`, `SAM2Transforms` | `feature_flag=1` | 针对train/test目录结构 |
| `update_edge_maps_from_npz.py` | 边缘修复 | 完全内部实现 | `cv2`, `pycocotools`, `numpy` | `edge_flag=1`, `edge_version='B_v1'` | 独立实现，不依赖其他模块 |
...
1. **特征修复**: 优先使用`bulk_fix_npz_features.py`进行批量修复，如果NPZ文件分散存储，使用`fix_npz_features_inplace.py`
2. **边缘修复**: 使用`update_edge_maps_from_npz.py`，支持多种过滤选项，适合大规模批量更新
