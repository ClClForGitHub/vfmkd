...
def _import_backbones():
    ...
    try:
        ...
        raise ImportError(f"导入 RepViTBackbone 失败: {e}") from e
    
    # YOLOv8 backbone 依赖 mmdet，仅在需要时导入（延迟到真正使用时）
    OfficialYOLOv8BackboneWrapper = None
    
    def _lazy_import_yolov8():
        """延迟导入 YOLOv8 backbone（仅在真正需要时调用）"""
        nonlocal OfficialYOLOv8BackboneWrapper
        if OfficialYOLOv8BackboneWrapper is not None:
            return OfficialYOLOv8BackboneWrapper
        
        try:
            from mmdet.registry import MODELS
        except ImportError as e:
            raise ImportError(
                f"使用 YOLOv8 backbone 需要安装 mmdet 和 mmcv，但导入失败: {e}\n"
                "如果不需要 YOLOv8 backbone，可以使用 --backbone repvit 或卸载 mmcv"
            ) from e
        
        class OfficialYOLOv8BackboneWrapper(nn.Module):
            """包装官方 YOLOv8CSPDarknet，提供与本地实现兼容的接口
            
            关键修复：必须继承 nn.Module 才能被 torch.compile 优化
            """
            def __init__(self, model_size='s'):
                super().__init__()  # 必须调用父类初始化
                # 根据 model_size 映射参数，对齐 Ultralytics 官方定义
                size_to_params = {
                    'n': (0.33, 0.25),
                    's': (0.33, 0.50),
                    'm': (0.67, 0.75),
                ...
                deepen_factor, widen_factor = size_to_params[model_size]
                ...
                backbone_cfg = dict(
                    ...
                    out_indices=(1, 2, 3, 4),  # 关键：获取 4 层输出 [S4, S8, S16, S32]
                    norm_cfg=dict(type='BN', momentum=0.03, eps=0.001),  # 官方标准 BN 参数
            ...
            def forward(self, x: torch.Tensor):
                """前向传播，返回与本地实现相同的格式（改用 forward 而不是 __call__）"""
    ...
    # 返回一个包装函数，只有在真正需要时才导入
    class LazyYOLOv8Wrapper:
        """延迟导入的 YOLOv8 wrapper"""
        def __call__(self, model_size='s'):
            cls = _lazy_import_yolov8()
            return cls(model_size=model_size)
    
    return LazyYOLOv8Wrapper(), RepViTBackbone
...
# 全局变量，在首次使用时初始化
_YOLOv8Backbone = None
_RepViTBackbone = None
_OTHER_MODULES = None
...
class DistillSingleTester:
    def __init__(self, config: dict, device: torch.device = None) -> None:
        # 延迟导入：在真正需要时才导入模块
        global _YOLOv8Backbone, _RepViTBackbone, _OTHER_MODULES
        if _YOLOv8Backbone is None or _RepViTBackbone is None:
            _YOLOv8Backbone, _RepViTBackbone = _import_backbones()
        if _OTHER_MODULES is None:
            _OTHER_MODULES = _import_other_modules()
        
        # 将导入的模块绑定到类属性，方便后续使用
        self.YOLOv8Backbone = _YOLOv8Backbone
        self.RepViTBackbone = _RepViTBackbone
    ...
    def _create_backbone(self) -> nn.Module:
        ...
        if backbone_type == "yolov8":
            # 延迟导入 YOLOv8 backbone（需要 mmdet/mmcv）
            if self.YOLOv8Backbone is None:
                YOLOv8BackboneClass = _import_yolov8_backbone()
                # 缓存导入的类
                self.YOLOv8Backbone = YOLOv8BackboneClass
            else:
                YOLOv8BackboneClass = self.YOLOv8Backbone
            model_size = self.config.get("model_size", "s")
            return YOLOv8BackboneClass(model_size=model_size)
        elif backbone_type == "repvit":
            return self.RepViTBackbone({
                "arch": "m1",
                "img_size": 1024,
                "fuse": False,
...
def main():
    ...
    if resume_checkpoint is not None:
        ...
        def load_component(key: str, module: nn.Module, friendly: str) -> None:
            ...
            # 兼容性检测：旧版 YOLOv8Backbone 权重无法直接加载到官方实现
            if friendly == "Backbone":
                module_class = module.__class__.__name__
                first_key = next(iter(state_dict.keys()), "")
                if module_class == "OfficialYOLOv8BackboneWrapper" and first_key.startswith("csp_darknet"):
                    raise RuntimeError(
                        "检测到旧版 YOLOv8Backbone 权重（包含 'csp_darknet' 前缀）。"
                        " 官方 MMDetection YOLOv8CSPDarknet 无法直接加载该权重，请使用新架构重新训练或转换权重。"
                    )
            try:
            ...
            if should_save:
                ...
                clean_checkpoint = {
                    "meta": {
                        ...
                        "backbone": config.get("backbone", "yolov8"),
