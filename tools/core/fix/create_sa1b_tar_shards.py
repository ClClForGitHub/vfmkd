...
try:  # pragma: no cover - lint 环境可能缺少 tqdm
...
class _DummyProgressBar:
    """Fallback 进度条，避免 tqdm 缺失时报错"""

    def __init__(self, iterable=None, total=None, initial=0, **_):
...
def _compute_signature(features_dir: str, images_dir: str, npz_files: List[str], image_index: Dict[str, str]) -> Dict[str, str]:
    """根据路径和文件列表生成签名，用于缓存校验"""
    hasher = hashlib.md5()
...
def _cache_path_for_signature(output_dir: str, signature: Dict[str, str]) -> str:
    suffix = signature["npz_hash"][:8]
    return os.path.join(output_dir, f".pairs_cache_{suffix}.json")


def _load_pairs_cache(cache_path: str, signature: Dict[str, str]) -> List[Tuple[str, str, str]] | None:
    ...
    try:
        ...
        if data.get("signature") != signature:
            return None
        return [tuple(item) for item in data.get("pairs", [])]
    except Exception:
        return None


def _save_pairs_cache(cache_path: str, signature: Dict[str, str], pairs: List[Tuple[str, str, str]]) -> None:
    try:
        payload = {
            "signature": signature,
            "pairs": pairs,
...
def match_pairs(npz_files: List[str], image_index: Dict[str, str], max_samples: int = None) -> Tuple[List[Tuple[str, str, str]], List[str]]:
    ...
    for npz_path in _progress_iter(iterator, desc="Matching", unit="file"):
...
def create_tar_shards(
    ...
    # 2. 匹配（支持缓存）
    signature = _compute_signature(features_dir, images_dir, npz_files, image_index)
    cache_path = _cache_path_for_signature(output_dir, signature) if use_cache else None
    pairs = None
    missing: List[str] = []

    if use_cache and cache_path:
        cached_pairs = _load_pairs_cache(cache_path, signature)
        if cached_pairs is not None:
            pairs = cached_pairs
            print(f"[Cache] 缓存命中，跳过匹配阶段 ({len(pairs)} 对)")
    ...
    if pairs is None:
        ...
        if use_cache and cache_path:
            _save_pairs_cache(cache_path, signature, pairs)
            print(f"[Cache] 匹配结果已缓存: {cache_path}")
    ...
    num_shards = (total_pairs + shard_size - 1) // shard_size
    ...
    if skipped_samples:
        skipped_shards = len(existing_indices & set(range(num_shards)))
        print(f"[Resume] 自动跳过 {skipped_samples} 条已存在的样本（对应 {skipped_shards} 个 shard）")
    ...
    total_written = 0
    errors = []
    completed_samples = total_pairs - total_target_samples
...
def main():
    ...
    parser.add_argument("--disable-cache", action="store_true",
                        help="不使用 pair 缓存（默认启用，用于加速多次运行）")
