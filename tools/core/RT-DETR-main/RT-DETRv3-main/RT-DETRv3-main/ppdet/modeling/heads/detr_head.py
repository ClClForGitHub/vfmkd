...
class DINOv3Head(nn.Layer):
    ...
    def forward(self, out_transformer, body_feats, inputs=None):
        ...
        if self.training:
            ...
            if dec_out_bboxes.shape[0] > 1:
                return {
                    'pred_logits': dec_out_logits[self.eval_idx],
                    'pred_boxes': dec_out_bboxes[self.eval_idx],
                    'aux_outputs': [
                        {'pred_logits': a_logits, 'pred_boxes': a_bboxes}
                        for a_logits, a_bboxes in zip(dec_out_logits, dec_out_bboxes)
                    ]
                }
            return (dec_out_bboxes[self.eval_idx],
                    dec_out_logits[self.eval_idx], None)
